...
**Architecture:** A fixed dynamic command scavenges only a valid expired same-session entry, then prepares an owner-only TTL-bounded slot bound to native `${CLAUDE_SESSION_ID}` and returns the runtime identity `{sessionId,requestId,capability}`. The adapter attempts to write one `soma-entry-request/v1` containing that identity and exact `$ARGUMENTS` as `rawArguments`; its `finally` path composes fixed-shape consume or abort with the three format-validated runtime tokens. Consume opens and validates the lease and envelope without mutation, compares `argv = lease = envelope`, and only then claims. It reopens the slot and matches device, inode, size and content hash with the pre-claim read before parsing `rawArguments`. Host death may leave one inert TTL-bounded slot for deterministic next-prepare scavenging after expiry. Invalid structures fail `BROKER_CORRUPT` and remain untouched. No user request value enters Bash. Handoff publishes facts without a digest; resume inspection computes `continuityDigest` after publication, and continue recomputes every input before any project lock or write. Entry returns baseline facts, while the orchestrator creates one `T-BASELINE` after `READY` and an executor runs it through dispatch records.
...
Consume must implement this order: open the lease and envelope no-follow without mutation; validate canonical containment, schema, owner, mode, regular type, live expiry, size and formats; retain the envelope device, inode, size and exact content hash; compare `argv sessionId/requestId/capability = lease = envelope`; and fail without claim, cleanup or project/Git/run mutation on any mismatch. Preserve a mismatched slot for diagnosis or authorized abort. Only after equality may consume create the exclusive claim. Reopen through the retained safe descriptor or canonical no-follow path and compare device, inode, size and content hash with the pre-claim read. Post-claim drift fails closed before parsing `rawArguments`, and cleanup removes only the authenticated slot. Parse and route `rawArguments` only after that revalidation succeeds. Abort compares the same argv tuple with the lease before claiming without parse or route, then performs the same cleanup. Cleanup in both commands is idempotent, and abort itself is idempotent. A malformed token never reaches the filesystem; a well-formed mismatch removes nothing. Signal handlers enter the same cleanup path. Host death may leave one inert slot; after expiry, the next prepare scavenges it only if it remains structurally valid.
...
Expected: PASS; sessions A/B cannot cross, an envelope-only capability mismatch creates no claim and preserves the slot, a swap between pre-read and claim is detected before `rawArguments` parsing, one same-session consumer wins, replay fails, normal flows leave zero slot, crash residue is TTL-bounded and scavenged, stale cleanup preserves a newer lease, and the lease never predicts request fields.
