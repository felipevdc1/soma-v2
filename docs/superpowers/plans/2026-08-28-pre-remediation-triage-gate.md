...
test('triage contract defines deterministic clusters, evidence closure, schema, and decisions', () => {
  ...
  for (const [name, text] of Object.entries(s)) {
    ...
    assert.match(text, /acoplamento `low`, `medium` ou `high`/i, name);
    ...
    assert.match(text, /`GO` só é válido com no máximo três causas independentes/i, name);
    assert.match(text, /confiança `HYPOTHESIS` ou `unmappedCount>0` força `DEFER`/i, name);
...
test('all three normative surfaces retain coordinator boundary and dispatch-record authority', () => {
  ...
  for (const [name, text] of Object.entries({ constitution: s.constitution, claude: s.claude, codex: s.codex })) {
    ...
    assert.match(text, /O executor usa apenas parsers locais determinísticos.*só pode escrever o relatório/i, name);
...
test('decision table preserves every complete boundary input/output row', () => {
  ...
  const s = sources();
  for (const [name, text] of Object.entries(s)) {
    assert.ok(text.includes('Tabela decisória de entrada/saída (linhas normativas completas):'), name);
