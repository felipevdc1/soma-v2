...
existentes. Durante essa etapa não se executam testes, build ou install, e não
se alteram código ou configuração. O limite global de tokens por run fica fora
do escopo.
...
O agente deve consolidar identidades em clusters sem dupla contagem. Para cada
cluster, a saída registra a contagem, os arquivos de prova, a confiança
`VERIFIED`, `INFERENCE` ou `HYPOTHESIS`, a causa raiz ou causa candidata e o
...
1. A regra identifica os dois gatilhos: 10 ou mais identidades falhando e
   falhas herdadas ou compartilhadas cuja integração dependa de exceção.
2. A regra limita a triagem a um agente, uma tentativa e evidências existentes.
3. O contrato proíbe testes, build, install e alterações de código ou
   configuração durante a triagem.
4. A saída exige clusters sem dupla contagem, contagem, arquivos de prova,
   confiança válida, causa e risco de acoplamento.
5. O contrato permite `GO` somente sob os três limites de independência,
   acoplamento e conhecimento da causa.
6. Todo resultado fora desses limites produz `DEFER`, checkpoint/handoff
   durável e nenhuma correção automática.
7. Constituição, Claude e Codex expressam a mesma regra, sem gate novo no CLI.
8. O texto deixa explícito que o limite global de tokens e a full suite estão
...
cada critério observável, incluindo o limiar 10, o gatilho por exceção,
deduplicação, `GO`, `DEFER` e a proibição de efeitos colaterais. Registrar a
fase RED antes de alterar as fontes. Implementar então a regra nas fontes
canônicas e nos dois adapters, executar apenas esse teste de contrato e usar o
teste de paridade existente para verificar a propagação. A full suite não é
pré-requisito nem dependência desta mudança.
...
triagem só classifica evidências disponíveis no início da etapa.

## Self-review

O documento cobre contexto, decisão, alternativas, contrato, fluxo, oito
critérios numerados, arquivos previstos, TDD, limites e não objetivos. Os
termos de decisão estão restritos a `GO`, `DEFER`, `VERIFIED`, `INFERENCE` e
`HYPOTHESIS`; não há `TBD`, `TODO`, contradição ou dependência de execução de
testes, build ou install.
