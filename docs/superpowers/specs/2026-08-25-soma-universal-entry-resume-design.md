...
- The skill invokes fixed-shape `broker-prepare`, `broker-consume` and `broker-abort` CLI templates. Prepare returns the validated native session ID plus runtime-generated request ID and capability. Consume and abort interpolate only those three validated runtime tokens. Missing or malformed identity fails closed.
...
The broker protects against accidental cross-session access, path and shell injection, corrupt files and replay. Consume and abort bind cleanup to the exact runtime-issued `{sessionId,requestId,capability}` tuple. A malformed token fails before filesystem access; a well-formed mismatch fails before claim and removes nothing. A corrupt slot is diagnostic evidence and is never scavenged automatically. The broker is not a sandbox against a hostile process running as the same uid; such a process can already modify the user's files. Owner and mode checks define the normal process boundary without overstating it. The skill sets `disable-model-invocation: true`, so only an explicit user invocation can start it.
...
4. Consume validates all three token formats, authenticates the canonical broker root, session directory, slot containment and every parent component with `lstat` and `realpath`, then opens the regular lease and envelope with no-follow. It compares `argv sessionId/requestId/capability = lease = envelope`; any mismatch fails before claim or cleanup and preserves the slot. Abort performs the corresponding argv-to-lease check without parsing the envelope.
5. Only after consume establishes the triple identity does it create the atomic claim. It discards the pre-claim descriptor, authenticates the path chain again and opens the request anew through its canonical no-follow path. It compares the new bytes, device, inode, size and content hash with the pre-claim read. Drift fails closed, and cleanup is limited to the already authenticated slot. Only bytes from the new opening reach the `rawArguments` parser. Both commands use idempotent cleanup in `finally` after success or error; abort itself is also idempotent.
...
These are slash-command forms, not shell commands. The adapter copies the complete text to one envelope without classifying it. Prepare is a fixed dynamic command in the skill. Consume and abort are fixed-shape Bash calls from the adapter's `finally` path after the Write attempt. Their bytes differ by command and runtime identity, but their grammar is fixed:

```bash
node ~/.soma-v2/scripts/soma.cjs entry broker-prepare --session "${CLAUDE_SESSION_ID}"
node ~/.soma-v2/scripts/soma.cjs entry broker-consume --session "<validated-runtime-session-id>" --request-id "<validated-runtime-request-id>" --capability "<validated-runtime-capability>"
node ~/.soma-v2/scripts/soma.cjs entry broker-abort --session "<validated-runtime-session-id>" --request-id "<validated-runtime-request-id>" --capability "<validated-runtime-capability>"
```

`${CLAUDE_SESSION_ID}` is native runtime data, not user input, and must match `^[A-Za-z0-9][A-Za-z0-9._-]{0,127}$`. Prepare generates request ID as 32 lowercase hex characters and capability as 64 lowercase hex characters, then returns all three identity tokens to the adapter. Before composing consume or abort, the adapter validates the same regexes again. Only these bounded runtime tokens are interpolated. `$ARGUMENTS`, objective, request path, SOMA run ID, project, scope, handoff and digest are absent from Bash. The fixed tilde path resolves the installed CLI without a configurable command fragment. The `soma` command on `PATH` is not required. `soma entry` remains an internal adapter API and is not documented as another workflow entry.
...
Every directory is mode `0700`. Prepare creates both `lease.json` and an empty regular `request.json` with no-follow, exclusive creation and mode `0600` before the structured Write can run. It generates request ID from 128 random bits encoded as exactly 32 lowercase hex characters and capability from at least 256 random bits encoded as exactly 64 lowercase hex characters. The minimal lease contains only `sessionId`, `requestId`, `capability`, exact request path, bounded `ttlMs` and monotonic creation and expiry timestamps, all known at preparation time. Wall-clock adjustment never extends a lease. It contains no request mode or content hash.

Missing or malformed `${CLAUDE_SESSION_ID}` returns `SESSION_UNAVAILABLE` before broker access. Before creating a slot, prepare validates the current same-session entry. A live valid entry returns `BROKER_BUSY`, or the existing lease only when the slot is still empty, regular, owner/mode-correct and unexpired. A schema-valid, owner/mode-valid expired entry is first moved to a unique cleanup name by atomic rename/claim, then a new request directory is created with `O_EXCL`; prepare never overwrites in place. A malformed lease, wrong owner/mode, symlink or other invalid structure returns `BROKER_CORRUPT` with a diagnostic and remains untouched. Sessions A and B hash to distinct directories, and consume for B derives only B's directory; it never scans A's slots.

Preparation output gives the adapter the validated session ID, exact pre-created path, request ID and capability. The adapter copies `$ARGUMENTS` once as `rawArguments` and writes the returned identity unchanged into the envelope. Request path and `rawArguments` never enter Bash, environment overrides, command substitutions or shell redirections. Only the three format-validated runtime identity tokens enter the fixed-shape consume or abort call.
...
1. Validate the canonical broker root before opening either file. Derive the session directory and slot, prove their containment under that root, and check the root, session directory, slot and every parent component with `lstat` and `realpath`. Reject any symlink, owner or exact mode mismatch, non-directory parent or canonical escape. Only after the entire chain passes may consume open the lease and envelope as regular files with no-follow.
2. Validate lease and envelope schema, owner, exact `0600` mode, regular-file type, live monotonic-safe expiry, bounded size, token formats and `rawArguments` type. Record the exact envelope bytes, device, inode, size and content hash from this non-mutating pre-claim read.
3. Compare `argv sessionId/requestId/capability = lease = envelope`, require the opened request path to equal the lease path, and confirm that the lease remains live. A mismatch, including a capability that differs only in the envelope, fails before claim, parse or cleanup. It causes no project, Git or run mutation and leaves the slot intact for diagnosis or an authorized abort.
4. Create the exclusive `claim` with `O_EXCL` only after the triple identity agrees. Replay and a second consumer fail.
5. Close and do not reuse the pre-claim request descriptor. Revalidate the canonical root, containment and every path component, then open `request.json` again through the authenticated canonical path with no-follow. Revalidate regular-file type, owner and mode.
6. Compare the new opening's exact bytes, device, inode, size and content hash with the pre-claim record. If any fact drifted, fail closed before parsing or routing. Cleanup may remove only the slot authenticated before claim; it must not scan or remove another request.
7. Parse `rawArguments` only from the bytes returned by the new opening and route the request. Return their hash as internal `requestSha256`; the lease does not predict it.
...
3. The only variable shell values are the validated native `${CLAUDE_SESSION_ID}` and prepare-generated request ID and capability. All have fixed alphabets and lengths. No user-supplied request value enters Bash; no inferred or alternate identity exists.
...
| Broker root, session directory, slot or a parent component is a symlink, has wrong owner/mode or escapes canonical containment | fail before opening lease or envelope; no claim, cleanup or project access |
| Request bytes, device, inode, size or content hash change between pre-read and the new post-claim opening | `INVALID_ENTRY_REQUEST`; fail closed before `rawArguments` parsing; cleanup only the authenticated slot |
...
| Next prepare finds a schema-valid, owner/mode-valid expired slot | atomically rename/claim and remove it before creating a new slot; old cleanup cannot remove the new lease |
| Next prepare finds a malformed lease, wrong owner/mode, symlink or invalid structure | `BROKER_CORRUPT` plus diagnostic; do not remove or replace it automatically |
...
| Broker parent | A parent becomes a symlink or escapes the canonical broker root before lease/envelope opening | `lstat`/`realpath`, containment, owner and mode validation reject the chain before either file opens |
| Broker swap | The envelope is swapped after the non-mutating pre-read and before claim | Consume discards the old descriptor, revalidates the path chain, opens the canonical request path anew with no-follow and compares new bytes, device, inode, size and hash; drift fails before `rawArguments` parsing |
...
Preparation, consumption and abort require valid native `${CLAUDE_SESSION_ID}`, a 32-lowercase-hex request ID, a 64-lowercase-hex capability, distinct session directories, canonical containment, owner, mode, a pre-created regular slot, no-follow access and bounded monotonic-safe expiry. Malformed identity fails before filesystem access. Before consume opens the lease or envelope, it validates the canonical broker root, session directory and slot containment plus every parent component with `lstat` and `realpath`; symlinks, owner/mode errors and canonical escapes fail closed. It then opens regular files with no-follow, validates schema and format, records the envelope bytes, device, inode, size and content hash, and requires `argv sessionId/requestId/capability = lease = envelope`. Any mismatch fails before claim, parse or cleanup, preserves the slot for diagnosis or authorized abort, and causes no project, Git or run mutation. Only then may consume create the atomic claim. It must discard the old descriptor, revalidate the entire path chain and open the request anew through the authenticated canonical path with no-follow. The new bytes, device, inode, size and content hash must match the pre-claim record. Consume parses only the newly opened bytes. Post-claim drift fails closed, and cleanup is limited to the authenticated slot. Every flow in which the host continues after prepare runs consume after a successful Write or abort after a failed or rejected Write, and leaves zero slot. If the host dies first, at most one inert slot remains until expiry; it cannot be consumed or mutate project, Git or run state. The next same-session prepare scavenges it only when lease schema, owner and mode are valid and expiry has passed. Corrupt structures remain untouched with `BROKER_CORRUPT`. A delayed R1 consume or abort cannot touch R2 because all three runtime tokens must match before any claim or cleanup.
