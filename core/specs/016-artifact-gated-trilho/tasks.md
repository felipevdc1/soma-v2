...
| T-20 | `[WIRING]` **Poda obrigatória** (§B.10) — reescrever as instruções de transição do `soma-run.md` para "objetivo + invariantes", agora que report + state v2 carregam o determinismo. Meta: **≤300 linhas**. A de-prescription é só aqui e só nisso. **Onde está a massa, medido em `0b54e7c` (arquivo com 525 linhas, pós-T-18)**: os 12 blocos de step somam **260** · `## Worked example — invocação` **59** (a maior seção isolada) · `## 0. Bootstrap` **30** · `## Recovery Protocol` **24** · os 2 GATEs humanos **33** · resto **63** · header/Gaps/separadores **~56**. **Piso intransponível**: as **24 linhas** `**Gate:**`/`**Report:**` (2 por bloco × 12) que a T-18 instalou — sem elas o arquivo volta a descrever intenção em vez de invocar o primitivo, e a fase inteira perde o efeito. Consequência prática: 525→300 **não é aparar prosa**, é reescrever os blocos de ~22 para ~11 linhas em média **e** encarar o `Worked example`. ⚠️ **Não renomear, reordenar nem fundir bloco de step** — os 12 nomes são a mesma lista do `STEP_ORDER` hardcodado em `gate.cjs` e `resume.cjs`, e mexer quebra os dois **em silêncio**, sem teste acusando (ver nota do `plan.md`) | [SPEC:AC-01] [SPEC:AC-02] | `core/adapters/claude/commands/soma-run.md` | T-18, T-19 | DONE |
