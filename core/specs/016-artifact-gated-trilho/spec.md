...
- **O log JSONL do run** — declarado Out of Scope em 2026-08-17, ao fim da Wave 2. Ele aparece como requisito em **dois** lugares desta spec (a linha de *Monitoring* em §Non-Functional e os §Side Effects de `contracts/emit-step-report.md`) e **não existe em lugar nenhum**: sem schema, sem caminho de arquivo, sem contrato próprio. **Nenhum dos 13 ACs o exige** — o AC-02 e o AC-10 se satisfazem inteiramente com exit code e conteúdo do report, que é onde o motivo estruturado já vive. A evidência que fechou a decisão é empírica, não argumentativa: o smoke de ponta a ponta da T-19 passou **os 4 critérios de "Fase 2 pronta" do §F mais o caminho `SONAR_CLEAN`**, sem o log existir. Um side effect mandatado sem forma é pior que um ausente — **lê como requisito e não é executável**, e foi por isso que a executora da T-06 parou nele em vez de improvisar um formato. **Para reverter**: escreva um contrato próprio (schema + caminho + quem emite) e um AC que o exija; enquanto isso não houver, as duas menções acima descrevem intenção, não obrigação.
