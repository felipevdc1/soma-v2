...
| T-13 | **As duas mudanças acopladas**, medidas na T-10 e documentadas em `noise-floor.md`: (a) `context.cjs` reconhece `[P]` **com ou sem crase** — hoje exige crase e lê 204 ocorrências reais como zero, deixando o `parallel-collision` cego em 14 das 16 specs; (b) entrada da coluna `files` só conta como arquivo se **parecer path** (sem espaço, sem parêntese, sem `+`, e com `/` ou extensão conhecida). ⚠️ **Entrar só com (a) troca cegueira por ruído** — a simulação dá 99 colisões, e uma fatia vem de prosa na coluna `files`, não de defeito. Corpus dos dois lados para cada metade. **Critério numérico**: rodar contra 001-015 e cada achado tem que ser **classificável como real ou ruído com o motivo**, não apenas somar menos que 99. Atualizar o `noise-floor.md` com o número final | [SPEC:AC-15] [SPEC:AC-16] | `core/scripts/lib/spec-lint/context.cjs`, `core/scripts/__tests__/contract-check-parallel.test.cjs`, `core/scripts/__tests__/fixtures/spec-lint/parallel/`, `core/specs/017-soma-spec-lint/noise-floor.md` | T-10 | DONE |
