...
**O quinteto do instalador desta noite (T-08a/b/c, 2026-08-21) — a mesma doença, cinco formas**: `install.sh` nunca teve um teste de que as suas próprias referências são reais.
...
5. `computeShaSumsFromDir` (T-08c pt.3, achado pelo orquestrador revisando o pt.1) — `--soma-dir` pra um diretório **inexistente** devolvia `{}` em silêncio, exatamente a mesma entrada que reabre o bug do pt.1 (todo hook do SOMA no destino vira colisão). Consertado: `dir` ausente, ou casando zero dos basenames esperados, agora é falha, nomeando o diretório.

Os dois primeiros são fail-loud por acidente (`set -e` salvou); os três últimos eram fail-silent por design (um otimista demais, um pessimista demais, e um que reabre o otimista demais por outro caminho) até esta rodada de tasks. Nenhum dos cinco tinha um teste perguntando "essa referência que o script confia é real?" — é a pergunta que `install-sh-rsync-origins.test.cjs` e `detect-collisions-fail-loud.test.cjs`/`detect-collisions-soma-dir.test.cjs` fazem agora. O pt.3 em particular mostra que a doença não fecha na primeira correção — `--soma-dir` foi criado especificamente pra resolver o pt.1, e ainda assim reabria o mesmo buraco por uma via lateral (diretório ausente) que ninguém tinha perguntado.
