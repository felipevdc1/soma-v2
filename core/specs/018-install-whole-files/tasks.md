...
5. **Baseline**: **1378 / 1370 / 5 / 3**, reconfirmado por medição em `7461de8` (2026-08-21) — o número de `c3019f7` sobreviveu. **Depois da T-01 o baseline é `1414 / 1406 / 5 / 3`** (os mesmos + 36 testes novos); compare **listas nominais**, nunca contagem bruta.
   ...
   >
   > Decomposição medida dos 34: **3 eram bug de teste anterior à T-08** (toda entry tratada como bloco, sem filtrar por `kind` — `adapter-contract-d-c11-lint.test.cjs` ×2 casos, `bf-04-cbm-deprecation-reproducer.test.cjs` "AC-01", `contract-file-entry.test.cjs` "caso 1") e foram **corrigidos nesta mesma leva**, com prova bidirecional (cada um mutado pra confirmar que ainda acusa entry de bloco ausente/nova, depois restaurado byte-a-byte). Os **26 restantes** são o buraco de isolamento de `$HOME` descrito acima e **NÃO foram tocados** — viram task própria (candidata a T-10), autorizada pelo orquestrador. **O baseline correto a partir de agora é `1485 / 1451 / 31 / 3`**: os mesmos 5 de sempre (`doctor drift`, `CC-07`, `phase4a-regression`, `AC-13 BLOCK_CONFLICT`, `SANDBOX_VIOLATION`) **mais os 26 da categoria acima**. Compare por **nomes**, nunca só a contagem 31 — a lista nominal dos 26 está no relatório da T-08. ⚠️ **31 é modal, não garantido** — 4 rodadas seguidas deram 31/31/31/32; a 32ª foi `phase3-regression.test.cjs` flagrando o mesmo acoplamento de `$HOME` por um caminho a mais (testes rodam em paralelo por padrão e correm sobre o `~/.claude` real ao mesmo tempo). Detalhe em `plan.md` §Baseline a preservar.
   🟢 **Atualização pós-T-08b + T-08c**: T-08b (`install.sh:169`) e T-08c (`detect-collisions.cjs`) adicionaram 4 testes próprios (2+2), todos análise estática/fixture sintético — nenhum toca `$HOME` real. **Baseline agora: `1489 / 1455 / 31 / 3`** (mesmo flake ±1 do `phase3-regression` observado). Os 31 continuam sendo os mesmos 5 + os 26 da Categoria B — zero fail novo confirmado por nome (não só contagem) nas duas tasks.
...
| T-08b | `[WIRING]` **Task nova, levantada pela T-08, autorizada pelo Felipe.** `install.sh:169` fazia `rsync "${REPO_ROOT}/commands/"` apontando pra um diretório que a T-04 apagou — 3a ocorrência da mesma classe de bug que a T-08a já tinha visto no `:168` (hooks). Corrigido pra `core/adapters/claude/commands/` **com `--exclude=soma-run.md`**, porque o `rsync` ingênuo destruiria o `soma-run.md` real do Felipe (474 linhas, o laboratório que o AC-12 protege) — medido num destino temporário, nada real tocado. O exclude é literal no shell, amarrado por teste ao `excluded` do adapter (single source of truth), não uma segunda cópia da regra. Dois testes: (1) classe — toda origem `rsync "${REPO_ROOT}/X/"` em `install.sh` existe no repo, varrendo o arquivo inteiro (não só a linha `:169`, pra não convidar uma 4a ocorrência); (2) AC-12 vale pros dois mecanismos — o que `install-targets.json`'s `excluded` declara também está excluído do `rsync` do `install.sh`. Análise estática pura — `install.sh` nunca é executado. | [SPEC:AC-12] | `install.sh`, `core/scripts/__tests__/install-sh-rsync-origins.test.cjs` | T-08 | DONE — RED genuíno nos 2 testes antes do fix, GREEN depois. Mutações provadas nos dois sentidos: origem inexistente → RED (teste 1) → restore; `--exclude` removido → RED (teste 2, diverge do JSON) → restore. Zero fail novo na suíte cheia (1487/1453/31/3) |
| T-08c | `[WIRING]` **Task nova, levantada pela T-08b, autorizada pelo Felipe.** `install.sh:117` chamava `detect-collisions.cjs` sem `--soma-sums`, e `detect-collisions.cjs:122` fazia `shaSums = somaSumsPath ? ... : {}` — sem referência, **todo hook SOMA presente no destino** era reportado como colisão ("no shipped sha available for comparison"). Medido contra o `~/.claude/hooks` real do Felipe: **18** falsos positivos sem referência vs **2** reais com uma real (batendo com um `cmp` arquivo-a-arquivo independente). Com `FORCE_OVERWRITE=1`, isso vira 18 arquivos renomeados pra `.bak`, 16 deles idênticos ao que ia ser instalado. Fix: `--soma-dir=<dir>` novo, que computa os sha reais via `sha256File()` (já existia no módulo) a partir de arquivos de verdade, sem exigir um JSON de sums pré-gerado; `install.sh:117` passa `--soma-dir="${REPO_ROOT}/core/hooks"`. Modo degradado (nem `--soma-sums` nem `--soma-dir`) deixa de ser silencioso: CLI sai 1 com mensagem clara. Dois testes com fixture 100% sintético (nunca lê `~/.claude/hooks` real): (1) 2 hooks byte-idênticos ao repo + 1 modificado → exatamente 1 colisão nomeada; (2) sem nenhuma referência → falha alta, não trata tudo como colisão (prova negativa: `thermal-guard.cjs` não aparece no stdout). | [SPEC:AC-05] | `install/detect-collisions.cjs`, `install.sh`, `core/scripts/__tests__/detect-collisions-soma-dir.test.cjs` | T-08b | DONE — RED genuíno nos 2 testes antes do fix, GREEN depois. Mutações provadas nos dois sentidos: `--soma-dir` revertido pro `{}` antigo → RED (teste 1, 3 colisões em vez de 1) → restore; guarda de falha alta removido → RED (teste 2, mensagem de erro genérica em vez de citar `--soma-sums`/`--soma-dir`) → restore. Suíte existente `install/__tests__/detect-collisions.test.cjs` (6 testes) continua verde, sem regressão. Zero fail novo na suíte cheia (1489/1455/31/3, mesmo flake ±1 conhecido do `phase3-regression`) |
