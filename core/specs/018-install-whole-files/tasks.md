...
5. **Baseline**: **1378 / 1370 / 5 / 3**, reconfirmado por medição em `7461de8` (2026-08-21) — o número de `c3019f7` sobreviveu. **Depois da T-01 o baseline é `1414 / 1406 / 5 / 3`** (os mesmos + 36 testes novos); compare **listas nominais**, nunca contagem bruta.
   ...
   🔴 **A partir da T-07 o baseline de falhas é 7, não 5** — as 5 pré-existentes **mais** os 2 do "frozen libs invariant" (`frozen-libs-invariant-014.test.cjs:43` e `manifest.test.cjs:345`), que acendem porque a D-018-05 autoriza tocar `core/scripts/lib/manifest.cjs`. Os dois comparam `git diff main -- core/scripts/lib/` e **voltam ao verde no merge, sem ação**. É tripwire funcionando, não regressão. Confira pelos **nomes**, nunca pela contagem. As 5 pré-existentes (`doctor drift`, `CC-07`, `phase4a-regression`, `AC-13 BLOCK_CONFLICT`, `SANDBOX_VIOLATION`) **não devem ser consertadas**. Meça antes e depois e reconcilie; nenhum fail NOVO fora dos seus.
...
| T-07 | `[P]` `sync.cjs` processa entry de arquivo — **duas passadas** (avalia todas sem escrever; só escreve se nenhuma divergiu), cópia byte-a-byte, coexistência com as 8 entries de bloco **sem alterar o comportamento delas**, e o invariante de propriedade: só toca o que uma entry declara. Usa fixtures próprios de `install-targets`, não o conjunto real (que é da T-08). Integration test `// @spec AC-01` `// @spec AC-02` `// @spec AC-03` `// @spec AC-04` `// @spec AC-05` | [SPEC:AC-01] [SPEC:AC-02] [SPEC:AC-03] [SPEC:AC-04] [SPEC:AC-05] | `core/scripts/sync.cjs`, **`core/scripts/lib/manifest.cjs`** (ver D-018-05), `core/scripts/__tests__/sync-file-entries.test.cjs` | T-02, T-03 | TODO |
