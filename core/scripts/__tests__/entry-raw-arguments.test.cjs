...
test('raw argument parser supports help, status, start, and resume public forms', () => {
  ...
  assert.deepEqual(parseRawArguments('--status'), { mode: 'status', project: null });
  assert.deepEqual(parseRawArguments('ship it'), { mode: 'start', objective: 'ship it', project: null });
  assert.deepEqual(parseRawArguments('--resume run-42 --project /tmp/project'), { mode: 'resume', runId: 'run-42', project: '/tmp/project' });
  assert.deepEqual(parseRawArguments('--resume'), { mode: 'resume', runId: null, project: null });
});
