...
Para `READY`, crie `runId = run-{YYMMDD-HHmm-xxxxxx}` e inicialize o estado com `soma run state --init --run <runId>`. Para `RESUME_READY`, use exatamente `runId`, `reentryState` e `nextTask` retornados. Não redescubra nem repita task `passed`.
